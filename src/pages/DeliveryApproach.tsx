...
import {
  ExternalLink, ArrowRight, Monitor, Zap, Settings, Rocket,
  Layers, Shield, Globe, CheckSquare, ListChecks, Cpu, Bot,
  GitBranch, BarChart2, AlertTriangle, Users, FileText,
...
const STRIP_ICONS: Record<string, React.ReactNode> = {
  monitor: <Monitor className="h-4 w-4" />, zap: <Zap className="h-4 w-4" />,
  settings: <Settings className="h-4 w-4" />, rocket: <Rocket className="h-4 w-4" />,
  layers: <Layers className="h-4 w-4" />, shield: <Shield className="h-4 w-4" />,
  globe: <Globe className="h-4 w-4" />, checkSquare: <CheckSquare className="h-4 w-4" />,
  cpu: <Cpu className="h-4 w-4" />, bot: <Bot className="h-4 w-4" />,
  gitBranch: <GitBranch className="h-4 w-4" />, barChart: <BarChart2 className="h-4 w-4" />,
};

const phaseColors = ["bg-primary","bg-info","bg-success","bg-warning","bg-destructive"];

const keyControlIcons = [
  <ListChecks className="h-4 w-4" />, <AlertTriangle className="h-4 w-4" />,
  <Users className="h-4 w-4" />, <Shield className="h-4 w-4" />,
];
const outputIcons = [
  <FileText className="h-5 w-5" />, <AlertTriangle className="h-5 w-5" />,
  <ClipboardList className="h-5 w-5" />,
];
const deliverableIcons = [
  <Shield className="h-5 w-5" />, <Users className="h-5 w-5" />,
  <Zap className="h-5 w-5" />, <ThumbsUp className="h-5 w-5" />,
];
const trackIcons = [
  <ClipboardList className="h-5 w-5" />, <AlertTriangle className="h-5 w-5" />,
  <Users className="h-5 w-5" />, <Cpu className="h-5 w-5" />,
];
const statusVariants: ("on-track" | "conditional" | "blocked")[] = ["on-track","conditional","blocked"];
const digitalServiceIcons = [
  <Monitor className="h-5 w-5" />, <BarChart2 className="h-5 w-5" />,
  <Rocket className="h-5 w-5" />, <GitBranch className="h-5 w-5" />,
];

const fadeUp = (delay = 0) => ({
  initial: { opacity: 0, y: 12 }, whileInView: { opacity: 1, y: 0 },
  viewport: { once: true }, transition: { duration: 0.35, delay },
});
    ...
    <Layout>
      ...
      <div className="border-b border-border bg-background">
        <AnimatePresence mode="wait">
          <motion.div key={activeTab + "-strip"} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}
            className="container mx-auto px-4 py-3 md:px-6">
            <div className="flex flex-col gap-2 md:hidden">
              {stripItems.map((item, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="shrink-0 text-primary">{STRIP_ICONS[item.icon]}</span><span>{item.label}</span>
                </div>
              ))}
            </div>
            ...
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="border-y border-border bg-background -mx-4 px-4 py-8 md:py-10 mt-6 md:mt-8" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "18px 18px" }}>
                <div className="mx-auto max-w-2xl text-center">
                  <svg width="28" height="22" viewBox="0 0 32 24" fill="none" className="mx-auto mb-3 opacity-20" aria-hidden="true"><path d="M0 24V14.4C0 6.4 4.8 1.6 14.4 0l1.6 2.4C10.4 3.6 7.6 6.4 7.2 10.4H12V24H0zm20 0V14.4C20 6.4 24.8 1.6 34.4 0L36 2.4C30.4 3.6 27.6 6.4 27.2 10.4H32V24H20z" fill="currentColor" className="text-primary"/></svg>
                  <p className="text-base font-medium leading-relaxed text-foreground md:text-lg">"The best digital tools are invisible to the end user — they just work."</p>
                </div>
              </div>
                ...
                {digitalItems.map((item, i) => (
                  <motion.div key={i} {...fadeIn(i * 0.08)} {...hoverLiftSm} className="rounded-lg border border-border bg-card p-4 card-shadow md:p-5">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary md:h-10 md:w-10">{digitalServiceIcons[i]}</div>
            ...
            <SectionWrapper className="relative overflow-hidden bg-accent/50" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2 md:mt-6">
                {digitalStack.map((badge, i) => (
                  <motion.span key={i} {...fadeIn(i * 0.04)} className="rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-card-foreground card-shadow md:text-sm">{badge}</motion.span>
                ))}
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <motion.div {...fadeIn(0)} className="relative text-center">
                ...
                <div className="mt-5 flex flex-col items-center gap-2.5 sm:flex-row sm:justify-center md:mt-6 md:gap-3">
                  <a href="https://cal.com/gagan.singh/15min" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
                    <Button size="lg" className="w-full font-semibold sm:w-auto">{t("hero.bookConsultation")} <ExternalLink className="ml-2 h-4 w-4" /></Button>
                  </a>
                  <Link to="/contact" className="w-full sm:w-auto">
                    <Button variant="outline" size="lg" className="w-full sm:w-auto">{t("cta.getInTouch")} <ArrowRight className="ml-2 h-4 w-4" /></Button>
                  </Link>
                </div>
              </motion.div>
            </SectionWrapper>
          ...
          <motion.div key="sap" {...tabVariants}>
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="border-y border-border bg-background -mx-4 px-4 py-8 md:py-10 mt-6 md:mt-8" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "18px 18px" }}>
                <div className="mx-auto max-w-2xl text-center">
                  <svg width="28" height="22" viewBox="0 0 32 24" fill="none" className="mx-auto mb-3 opacity-20" aria-hidden="true"><path d="M0 24V14.4C0 6.4 4.8 1.6 14.4 0l1.6 2.4C10.4 3.6 7.6 6.4 7.2 10.4H12V24H0zm20 0V14.4C20 6.4 24.8 1.6 34.4 0L36 2.4C30.4 3.6 27.6 6.4 27.2 10.4H32V24H20z" fill="currentColor" className="text-primary"/></svg>
                  <p className="text-base font-medium leading-relaxed text-foreground md:text-lg">"A go-live isn't the finish line. It's where the real work begins."</p>
                </div>
              </div>
              <div className="mt-6">
            ...
            <SectionWrapper className="bg-accent/50" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="grid grid-cols-2 gap-2 md:hidden">
                {uatPhases.map((phase, i) => (<motion.div key={i} {...hoverLiftSm} className="rounded-lg border border-border bg-card p-3 card-shadow"><h3 className="text-xs font-semibold text-card-foreground">{phase.name}</h3><p className="mt-1 text-xs leading-snug text-muted-foreground">{phase.desc}</p></motion.div>))}
              </div>
              <div className="hidden md:block">
                <div className="mb-5 hidden overflow-x-auto lg:flex">
                  {uatPhases.map((phase, i) => (<div key={i} className="flex items-center"><div className="flex h-10 items-center rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground">{phase.name}</div>{i < uatPhases.length - 1 && <ArrowRight className="mx-1.5 h-4 w-4 text-muted-foreground" />}</div>))}
                </div>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  {uatPhases.map((phase, i) => (<motion.div key={i} {...fadeIn(i * 0.06)} {...hoverLiftSm} className="rounded-lg border border-border bg-card p-4 card-shadow"><h3 className="text-sm font-semibold text-card-foreground">{phase.name}</h3><p className="mt-2 text-xs text-muted-foreground">{phase.desc}</p></motion.div>))}
                </div>
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                {keyControls.map((c, i) => (<motion.div key={i} {...fadeIn(i * 0.06)} {...hoverLiftSm} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 card-shadow md:p-4"><div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary md:h-8 md:w-8">{c.icon}</div><span className="text-xs font-medium text-card-foreground md:text-sm">{c.label}</span></motion.div>))}
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper className="bg-accent/50" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="grid gap-3 md:grid-cols-3 md:gap-4">
                {outputItems.map((o, i) => (<motion.div key={i} {...fadeIn(i * 0.08)} {...hoverLiftSm} className="rounded-lg border border-border bg-card p-4 card-shadow md:p-5"><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-accent text-primary md:h-10 md:w-10">{o.icon}</div><h3 className="text-sm font-semibold text-card-foreground">{o.title}</h3></div><p className="mt-2.5 text-xs text-muted-foreground md:text-sm">{o.desc}</p></motion.div>))}
              </div>
            </SectionWrapper>
            ...
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-5">
                {trackItems.map((item, i) => (<motion.div key={i} {...fadeIn(i * 0.08)} {...hoverLiftSm} className="rounded-lg border border-border bg-card p-3 card-shadow md:p-5"><div className="flex items-center gap-2"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-accent text-primary md:h-10 md:w-10">{item.icon}</div><h3 className="text-xs font-semibold text-card-foreground md:text-base">{item.title}</h3></div><p className="mt-2 text-xs text-muted-foreground md:mt-3 md:text-sm">{item.desc}</p></motion.div>))}
              </div>
            </SectionWrapper>
          ...
          <motion.div key="quality" {...tabVariants}>
            <SectionWrapper className="relative overflow-hidden" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.05) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="border-y border-border bg-background -mx-4 px-4 py-8 md:py-10 mt-6 md:mt-8" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "18px 18px" }}>
                <div className="mx-auto max-w-2xl text-center">
                  <svg width="28" height="22" viewBox="0 0 32 24" fill="none" className="mx-auto mb-3 opacity-20" aria-hidden="true"><path d="M0 24V14.4C0 6.4 4.8 1.6 14.4 0l1.6 2.4C10.4 3.6 7.6 6.4 7.2 10.4H12V24H0zm20 0V14.4C20 6.4 24.8 1.6 34.4 0L36 2.4C30.4 3.6 27.6 6.4 27.2 10.4H32V24H20z" fill="currentColor" className="text-primary"/></svg>
                  <p className="text-base font-medium leading-relaxed text-foreground md:text-lg">"Catching a defect in testing costs a fraction of what it costs in production."</p>
                </div>
              </div>
              <div className="mt-6">
                <motion.div {...fadeIn(0)}>
            ...
            <SectionWrapper className="relative overflow-hidden bg-accent/50" style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.04) 1px, transparent 1px)", backgroundSize: "20px 20px" }}>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 md:mt-6 md:grid-cols-4 md:gap-4">
                {deliverables.map((d, i) => (<motion.div key={i} {...fadeIn(i * 0.08)} {...hoverLiftSm} className="flex flex-col items-center gap-2 rounded-lg border border-border bg-card p-3 text-center card-shadow md:flex-row md:gap-3 md:p-4 md:text-left"><div className="text-primary">{d.icon}</div><span className="text-xs font-medium text-card-foreground md:text-sm">{d.label}</span></motion.div>))}
              </div>
            </SectionWrapper>

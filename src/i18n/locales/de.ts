const de = {
  nav: {
    home: "Startseite",
    delivery: "Lieferansatz",
    uat: "UAT-Governance",
    release: "Release-Bereitschaft",
    about: "Über uns",
    contact: "Kontakt",
    bookCall: "Beratung buchen",
  },
  hero: {
    pills: ["SAP-Governance & Compliance", "Testautomatisierung & Qualitätssicherung", "Kanada HQ · Globale Partner"],
    title: "SAP S/4HANA Governance & Intelligente Automatisierung",
    subtitle: "Strukturierte UAT-Governance und Release-Bereitschaft für SAP-Programme — geliefert von einem kanadischen Team mit spezialisierten globalen Lieferpartnern.",
    reach: "Kundenbetreuung lokal in Kanada · Lieferung unterstützt durch geprüfte globale SAP-Partner",
    bookConsultation: "Beratung buchen",
    bookCall: "Gespräch buchen",
  },
  capabilities: {
    sapDelivery: "SAP-fokussierte Lieferung",
    globalNetwork: "Kanada HQ · Globales Partnernetzwerk",
    clientOversight: "End-to-End-Kundenbetreuung",
    rapidStart: "Schneller Projektstart",
  },
  whatWeDeliver: {
    heading: "Was wir liefern",
    subheading: "Integriertes Know-how in Enterprise-Governance, Qualitätssicherung und digitaler Produktentwicklung.",
    cards: [
      {
        title: "SAP S/4HANA UAT-Governance",
        desc: "Strukturierte UAT-Programme mit definierten Ein-/Ausstiegskriterien, Geschäftsabnahmeprozessen und funktionsübergreifender Stakeholder-Koordination.",
      },
      {
        title: "Release-Bereitschaft & Qualitätstore",
        desc: "Metrikgesteuerte Go/No-Go-Entscheidungen anhand von Echtzeit-Fehlerkurven, Testabdeckungs-KPIs und risikobasierten Release-Kriterien.",
      },
      {
        title: "SAP-Teststrategie & Regressionstestabdeckung",
        desc: "Testplanung nach SAP-Best-Practices — abdeckend Kerngeschäftsprozesse, Integrationsszenarien und Fiori-Nutzerakzeptanz.",
      },
      {
        title: "Intelligente Testautomatisierung",
        desc: "Automatisierungsframeworks mit Cypress und Playwright sowie KI-unterstütztem Testdesign zur Reduktion manuellen Aufwands und Beschleunigung von Regressionszyklen.",
      },
      {
        title: "Webanwendungs- & Dashboard-Entwicklung",
        desc: "Maßgeschneiderte Webanwendungen, Reporting-Dashboards und Business-Tools — als ergänzende Fähigkeit neben SAP-Engagements geliefert.",
        detail: [
          "Geschäftliche Webanwendungen und Portale",
          "Reporting- und Betriebsdashboards",
          "Rapid Prototyping und Deployment",
          "UX-orientiertes, stakeholdergerechtes Design",
        ],
      },
    ],
  },
  howWeWork: {
    heading: "Wie wir arbeiten",
    subheading: "Ein einziger Ansprechpartner in Kanada, unterstützt durch ein spezialisiertes globales Partnernetzwerk — für erstklassige SAP-Expertise mit lokaler Verantwortung.",
    steps: [
      {
        title: "Lokal engagieren",
        desc: "Ihr fester Ansprechpartner sitzt in Kanada. Wir managen das gesamte Scoping, die Kommunikation und die Kundenbetreuung aus dem GTA.",
      },
      {
        title: "Experten-Partner einsetzen",
        desc: "Wir aktivieren geprüfte globale SAP-Spezialisten für Implementierung, Governance und Automatisierung — passend zu Ihren Programmanforderungen.",
      },
      {
        title: "Mit Vertrauen liefern",
        desc: "Strukturierte UAT-Governance, Qualitätstore und Release-Bereitschaft — mit voller Transparenz. Keine Überraschungen.",
      },
    ],
  },
  beyondSap: {
    heading: "Über SAP hinaus",
    body: "Wir bieten Testautomatisierung und Qualitäts-Governance für Enterprise- und moderne Plattformen, einschließlich SAP S/4HANA, Fiori, Webanwendungen und APIs mit Cypress und Playwright. Unser Ansatz vereint Teststrategie, Automatisierung und Release-Governance unter einem einheitlichen Betriebsmodell.",
    capabilities: [
      "SAP-Teststrategie und Regressionsautomatisierung",
      "Cypress- und Playwright-Frameworks",
      "API- und Integrationstests",
      "CI/CD-Qualitätstore",
    ],
  },
  outcomes: {
    heading: "Geschäftliche Ergebnisse",
    items: [
      { label: "Schnellere Regressionszyklen", desc: "Automatisierte Abdeckung reduziert manuellen UAT-Aufwand erheblich" },
      { label: "Reduziertes Release-Risiko", desc: "Strukturierte Qualitätstore verhindern kostspielige Post-Go-Live-Probleme" },
      { label: "Klare Go/No-Go-Entscheidungen", desc: "Echtzeit-Fehlermetriken beseitigen Rätselraten bei Releases" },
      { label: "Erhöhtes Vertrauen der Stakeholder", desc: "Stakeholder genehmigen Releases mit voller Transparenz" },
    ],
  },
  cta: {
    heading: "Bereit, Ihre Release-Qualität zu stärken?",
    body: "Lassen Sie uns besprechen, wie strukturierte SAP-Test-Governance und moderne Webentwicklung Ihr Release-Risiko reduzieren können — mit lokaler Betreuung in Kanada und spezialisierten globalen Lieferpartnern.",
    bookConsultation: "Beratung buchen",
    viewApproach: "Unsere Partner kennenlernen →",
  },
  delivery: {
    heading: "Lieferansatz",
    subheading: "Ein strukturierter Lebenszyklus von der Planung bis zur Hypercare — Qualität in jeder Phase einbetten.",
    phases: [
      { name: "Plan", desc: "Teststrategie & Scope-Abstimmung mit Geschäftsstakeholdern." },
      { name: "SIT", desc: "Automatisierungsorientierte Regressionsplanung und strukturierte SIT-Durchführung." },
      { name: "UAT", desc: "UAT-Koordination, Geschäftsabnahmen und Fehler-Governance." },
      { name: "Release", desc: "Risikobasierte Release-Governance mit Qualitätstor-Durchsetzung." },
      { name: "Hypercare", desc: "Überwachung nach dem Go-live, Fehler-Triage und Stabilisierung." },
    ],
    automationHeading: "Testautomatisierungsstrategie",
    automationSubheading: "Plattformübergreifende Automatisierung über SAP-, Web- und API-Ebenen mit modernen Frameworks.",
    automationItems: [
      "Automatisierungsorientierte Regressionsplanung",
      "Cypress- und Playwright-Implementierung",
      "SAP-Automatisierungsabstimmung",
      "CI/CD-Integration und Qualitätstore",
    ],
    coreCapabilities: "Kernkompetenzen",
    capabilities: [
      "Testplanung für ERP-Programme",
      "Fehler-Triage-Leitung",
      "Stakeholder-Reporting & Metriken",
      "CI/CD-Qualitätstore",
      "KI zur Testoptimierung",
    ],
  },
  uat: {
    heading: "UAT-Governance-Framework",
    subheading: "Ein strukturiertes Betriebsmodell für UAT — von der Planung bis zur Abnahme — zur Sicherstellung der Geschäftsbereitschaft und Qualitätsausrichtung.",
    operatingModel: "UAT-Betriebsmodell",
    phases: [
      { name: "Plan", desc: "UAT-Scope, Zeitpläne, Ressourcenbedarf und Eingangskriterien definieren." },
      { name: "Scope", desc: "Testszenarien auf Geschäftsprozesse und Risikobereiche abstimmen." },
      { name: "Durchführen", desc: "UAT-Zyklen mit strukturiertem Tracking und Fehlerdokumentation ausführen." },
      { name: "Triage", desc: "Tägliche Fehler-Triage mit prioritätsbasierter Lösung und Eskalation." },
      { name: "Abnahme", desc: "Formelle Geschäftsabnahmen gegen Ausgangskriterien einholen." },
    ],
    keyControls: "Schlüsselkontrollen",
    controls: ["Ein- und Ausgangskriterien", "Tägliche Fehler-Triage", "Abstimmung mit Geschäftsstakeholdern", "Scope- und Änderungskontrolle"],
    outputs: "Ergebnisse für Stakeholder",
    outputItems: [
      { title: "UAT-Statusberichte", desc: "Regelmäßiges Reporting zu Ausführungsfortschritt, Fehlerkurven und Risikobereichen." },
      { title: "Risikoprotokolle", desc: "Dokumentierte Risiken mit Schweregrad, Verantwortung und Maßnahmen." },
      { title: "Go-Live-Bereitschaftszusammenfassung", desc: "Konsolidierte Übersicht von Abnahmen, offenen Punkten und Release-Empfehlung." },
    ],
    deliverablesHeading: "Was starke UAT-Governance liefert",
    deliverables: ["Klare Scope-Kontrolle", "Geschäftliche Einbindung", "Schnellere Fehlerauflösung", "Go-live-Vertrauen"],
  },
  release: {
    heading: "Release-Bereitschafts-Framework",
    subheading: "Ein strukturiertes Governance-Modell für SAP S/4HANA-Programme — Testabdeckung, Fehlerbehebung und Geschäftsbereitschaft in einer kontrollierten, risikobasierten Release-Entscheidung zusammenfassen.",
    reach: "Unterstützung von SAP-Implementierungspartnern in Europa und Nordamerika.",
    trackHeading: "Was wir verfolgen",
    trackItems: [
      { title: "Testausführungsstatus", desc: "Testfortschritt nach Scope-Bereichen — SIT, UAT, Regression — mit klaren Abschlusskriterien für jede Phase." },
      { title: "Fehler nach Schweregrad & Auswirkung", desc: "Überwachung offener Fehler nach Priorität und Geschäftsauswirkung, um kritische Probleme vor dem Release zu lösen." },
      { title: "Geschäftsabnahmen & Bereitschaft", desc: "Verfolgung von Stakeholder-Genehmigungen über Geschäftsbereiche mit klarer Verantwortung und Eskalationspfaden." },
      { title: "Automatisierungsabdeckung für Regression", desc: "Messung des Automatisierungsfortschritts gegen den Regressionsumfang zur nachhaltigen, wiederholbaren Testausführung." },
    ],
    decisionModel: "Release-Entscheidungsmodell",
    statuses: [
      { label: "GO", desc: "Alle Qualitätstore erfüllt. Keine kritischen Fehler. Geschäftsabnahmen abgeschlossen. Automatisierungsziele erreicht." },
      { label: "BEDINGTES GO", desc: "Geringfügige Risiken identifiziert. Einige Abnahmen ausstehend oder niedrigpriore Fehler verbleiben. Maßnahmen vorhanden." },
      { label: "KEIN GO", desc: "Kritische Blocker vorhanden. Offene P1/P2-Fehler, fehlende Abnahmen oder unzureichende Testabdeckung." },
    ],
    governanceHeading: "Wie Release-Entscheidungen gesteuert werden",
    governanceBody: "Jede Release-Empfehlung basiert auf strukturierten Qualitätstor-Bewertungen — Risikoindikatoren, offene Fehlerschwere, Geschäftsbereichsbereitschaft und Testabdeckungsschwellen werden ausgewertet. Die resultierende Go/No-Go-Position ist transparent, evidenzbasiert und formal mit den Abnahmeanforderungen der Geschäftsstakeholder abgestimmt.",
  },
  about: {
    heading: "Über uns",
    tags: ["SAP-Governance-Experte", "Automatisierungs- & Qualitätssicherungsspezialist", "Digitale Lösungen & Website-Entwickler"],
    bio: [
      "Ich bin ein Technology-Delivery- und Quality-Engineering-Profi mit über 10 Jahren Erfahrung in KI-gesteuertem DevTestOps, CI/CD-Transformation und automatisierungsorientierten Qualitätsstrategien. Ich helfe Organisationen, schneller und mit Vertrauen zu liefern, indem ich Qualität früher im Lieferlebenszyklus verankere.",
      "Mein Hintergrund umfasst Enterprise-Delivery in SaaS, ERP, Versicherung, HealthTech, FinTech und E-Commerce — mit Plattformen wie Workday, SAP S/4HANA, Salesforce und Microsoft Dynamics 365. Ich habe cross-regionale Programme in Europa, Asien und Nordamerika geleitet.",
      "Über Enterprise-Systeme hinaus entwerfe und entwickle ich moderne Websites und Webanwendungen — die Lücke zwischen komplexen Backend-Plattformen und polierten digitalen Erlebnissen schließend.",
      "Heute konzentriere ich mich auf Generative KI und LLMs zur Verbesserung des Testdesigns, Beschleunigung der Automatisierung, prädiktive Fehlererkennung und risikobasierte Release-Governance in global verteilten Teams.",
    ],
    focusHeading: "Schwerpunkte",
    focusAreas: [
      "SAP-Test-Governance",
      "KI für Qualitätssicherung",
      "DevTestOps-Transformation",
      "Risikobasiertes Release-Management",
      "Website- & Webanwendungsentwicklung",
      "Digitale Transformationsberatung",
    ],
  },
  contact: {
    heading: "Kontakt aufnehmen",
    subheading: "Bereit, Ihre Release-Qualität zu verbessern? Lassen Sie uns ins Gespräch kommen.",
    email: "E-Mail",
    bookCall: "Gespräch buchen",
    location: "Standort",
    formHeading: "Nachricht senden",
    namePlaceholder: "Ihr Name",
    emailPlaceholder: "sie@unternehmen.com",
    messagePlaceholder: "Erzählen Sie mir von Ihrem Projekt...",
    nameLabel: "Name",
    emailLabel: "E-Mail",
    messageLabel: "Nachricht",
    send: "Nachricht senden",
    sending: "Wird gesendet…",
    successTitle: "Nachricht gesendet!",
    successDesc: "Vielen Dank für Ihre Anfrage. Ich melde mich bald.",
    errorTitle: "Senden fehlgeschlagen",
    errorDesc: "Etwas ist schiefgelaufen. Bitte senden Sie direkt eine E-Mail.",
  },
  footer: {
    tagline: "Enterprise-Governance, Testautomatisierung und moderne Webentwicklung — End-to-End-Digitallösungen für sichere Lieferung.",
    quickLinks: "Schnellzugriff",
    contact: "Kontakt",
    rights: "Alle Rechte vorbehalten.",
    translationNote: "Einige Inhalte auf dieser Website wurden möglicherweise automatisch übersetzt und können Ungenauigkeiten enthalten. Die englische Version ist die offizielle Version.",
  },
  lang: {
    en: "Englisch",
    de: "Deutsch",
    fr: "Französisch",
  },
};

export default de;
